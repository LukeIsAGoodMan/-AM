...
## D23 (P17) — Multi-cap per rule via `caps` jsonb array + calculator applies all caps

**Decision**: `reward_rules.caps` becomes a `jsonb NOT NULL DEFAULT '[]'` column holding an array of `{usageKey, basis, period, amountHkd?, rewardAmount?}` records. Migration 0013 backfills existing single-cap rows into `caps[0]` and drops the five flat cap_* columns (cap_amount_hkd, cap_reward_amount, cap_period, cap_basis, cap_usage_key). `ResolvedRule.cap: ResolvedCap | null` becomes `ResolvedRule.caps: ResolvedCap[]`. Calculator's `applyRuleWithCap` iterates: every spending-basis cap clips eligible spend (tightest wins), then `applyFormula` runs, then every reward-basis cap clips the resulting reward. Materializer's `loadMatchingCaps` now returns an array — primary (exact category_slug OR applies_to fan-out) plus card-level `cap_default=*_card_level` if the card has one. YAML syncer still reads a single `cap:` object from `data/cards/*.yaml` and writes it as `caps: [singleCap]`; multi-cap YAML is not modelled yet.

**Why**:
- **Real T&C has multi-tier caps**: Amex Explorer's live rules read "6x points on airlines, up to HK$10k spend per quarter" AND "aggregate annual spending cap HK$120k across all category bonuses". Both bind. Pre-P17 the materializer picked the tighter one and dropped the other (P15's "first hit wins" precedence). For a rule that hit both, the aggregate cap was silently ignored — the calculator overcredited across-category-spend that should have been bounded by the card-wide limit. Same shape on HSBC Premier (welcome-spend cap + welcome-period cap on the base earn), BOC Cheers, Citi Rewards. Fixing this properly required moving from one-cap-per-rule to N-caps-per-rule everywhere the calculator + storage layer touched it.
- **jsonb array over one-to-many table**: two options considered — (A) `caps` jsonb array on the rule row, (B) new `reward_rule_caps` table joined on `rule_id`. Went with (A) because: caps have no lifecycle independent of the rule (add/edit/delete are always parent-scoped), average N is 1-2, and every read path is "load the rule, iterate caps" — same query, one row. A join table would double the writes on every insert and cost a JOIN on every rule load without buying anything schema-wise. The jsonb is validated at the mapRow layer (parseCapsJson in resolved-rule.ts drops malformed entries silently — same tolerance as the aggregator's informational-field carve-out).
- **Backfill + drop in ONE migration**: atomic — either the caps column has valid data for every row and the old columns are gone, or nothing changed. Rolling forward through a "keep both columns during transition" phase would double-write everywhere and leave two sources of truth. Given only three writers (syncer, materializer, edit-rule action) and three readers (three mapRow copies + queries/rules.ts UI adapter), a hard cutover is safer.
- **Match order in loadMatchingCaps kept from P15**: (1) exact category_slug match → per-rule usageKey (`null`, mapRow falls back to slug), (2) applies_to fan-out → shared `xcap:<capGroupId>`, (3) card-level `cap_default=*_card_level` → ALWAYS also applies (change from P15's "only if no more-specific"). Rule (1) and (2) are mutually exclusive by design — a single earn_rate can't semantically have both a category-exact AND a fan-out cap. Rule (3) is orthogonal: card-wide caps compose with per-category caps.
- **Calculator: two-pass application**: spending caps first (they shrink eligible spend before applyFormula), reward caps second (they cap the formula's output). Mixed caps on the same rule combine correctly this way — HK$50k spending cap AND HK$500 reward cap on a 5% rule means: 60000 spend → 50000 eligible → 2500 reward pre-cap → 500 reward final. Any spending cap fully consumed → 0 reward; same for any reward cap.
- **UI backward-compat via derived getters**: `queries/rules.ts` still exposes `capAmountHkd / capPeriod / capBasis / capRewardAmount` on `RuleListRow` and `RuleWithCapCompat`, but they're computed from `caps[0]` at query time. The admin edit form still shows a single cap; on save, `buildCapsFromEdit` writes back to `caps[0]` while preserving `caps[1..]` — so editing an xchk__ rule with a primary + card-level secondary doesn't silently drop the secondary. Full multi-cap editing UI is deferred to whenever the operator needs it; for now the M15 form covers 90% of use.

**Knock-on**:
- **`caps` array validation is at mapRow, not schema**: Postgres doesn't enforce jsonb shape, and adding a CHECK constraint that JSON-schema-validates each element is overkill for a self-populated column. `parseCapsJson` in `src/lib/calculator/resolved-rule.ts` is the single validation point; if a rogue writer inserts malformed entries, they're dropped silently and logged in dev — same tolerance the aggregator applies to informational fields. If we ever grow a UI that lets users hand-write caps, tighten this to throw.
- **Live corpus: 33 xchk__ rules with at least one cap, all currently single-cap**. The multi-cap plumbing is there but the corpus doesn't exercise it yet — cards with both a category-specific cap AND a card-level cap (Amex Explorer, HSBC Premier, BOC Cheers, Citi Rewards, Amex Platinum) either have no matching xchk earn_rate group (all yaml-dedup'd via P16) or the LLM's category taxonomy doesn't align between the earn_rate and the cap (e.g. Citi Rewards' earn_rates are `dining_local/travel_hotel/department_store` but its applies_to caps target `entertainment` — parent-of-leaves mismatch). Both gaps are data-quality issues fixable in P18+ (category taxonomy harmonization) or by adding hand-curated YAML for these cards.
- **Calculator regression coverage**: four new tests in `calculate.test.ts` covering (a) two spending caps, tighter binds, (b) card-level fills up first, category-specific stays open — card-level binds now, (c) category cap consumed, zero reward even with card-level room, (d) mixed spending + reward cap on same rule. If the calculator regresses to picking one cap only, at least (b) fails.
- **Materializer test rewritten**: the D21 read-only pin (P15's "shared xcap: usageKey" invariant) now iterates `caps[]` on every xchk__ rule and asserts at least one has an `xcap:` prefix. Reset helper (`resetGroupMaterialization`) already guards yaml rules (D22) — no further change needed.
- **Simulator `naive.ts` iterates caps[]**: reset-period-token, cap advance, and the "conservative cap tracking" caveat all now walk `rule.caps` in a nested loop. Wall-clock impact is negligible (N=1 or 2 per rule, ~200 rules total).
- **Not modelled (P18+ candidates)**: (a) YAML multi-cap literal — extending the yaml schema so a curator can write `caps: [...]` directly instead of `cap: {...}`; (b) UI to edit N caps per rule; (c) category taxonomy harmonization (aggregator understands `applies_to=entertainment` ⊇ `category_slug=dining_local` etc.); (d) rules.ts UI adapter that shows all caps in the rule detail page, not just caps[0].
...
## D24 (P18 Stage 1A) — earn_rate grouped by formula (+currency); verdict uses shared structured comparator

**Decision**: `computeKeyDimension('earn_rate', …)` now appends a formula discriminator after a `|formula=` separator: `category_slug=dining_local|formula=simple_percent`, `rule_type=base_earn|formula=points_per_hkd:asia_miles`. The discriminator is `rewardFormulaType` (+ `:currencySlug` for point/mile formulas). Cap groups keep their category-only keys; the materializer's `loadMatchingCaps` strips the suffix via the new `stripFormulaSuffix()` so cap-stitch is unchanged. Separately, the aggregator's `claimAgreesWith` now delegates to `structuresAgree` (src/lib/normalize) instead of a hand-rolled `valuesAgree` — fixing the `String(object)` array bug and adding numeric-representation normalization.

**Why**:
- **§3A / §8 canary requirement**: Blue Cash has a 1.2% cashback base claim AND a HK$6=1 Asia Mile base claim (its alt reward mode). Both were `rule_type=base_earn` → same group → the formula-type mismatch made the group a permanent `conflict`, so the correct 1.2% base rule never materialized. Splitting by formula puts them in separate groups; each can then be judged (and the miles mode gated as a candidate, §3D). Reward currency is in the key so Explorer's `merchant_specific` amex_points 5x and its asia_miles expression of the same earn don't collapse — they carry different valuations.
- **Suffix on earn_rate only, caps untouched (§12 "P17 stays intact")**: caps are keyed by category (`category_slug=X`, `applies_to=…`, `cap_default=…_card_level`). Rather than re-key caps too, the earn_rate key carries the formula as a *suffix* the cap matcher strips. `stripFormulaSuffix` is identity on pre-P18 keys, so already-materialized non-canary rules keep stitching exactly as before. Cap-stitch semantics are byte-for-byte preserved.
- **structuresAgree over valuesAgree**: the old array branch did `String(x)` → every object became `"[object Object]"`, so distinct welcome components compared equal (§3 problem #3). Routing the whole claim through the §3B comparator fixes that AND normalizes numeric representations. Consequence: a numeric string now agrees with its number (`"1"` == `1`, `"100000.00"` == `100000`) — the old "no auto-coerce mixed types" rule contradicted §3B point 1 and is dropped. A non-numeric string vs a number is still a real conflict. In the jsonb→jsonb aggregator path numbers stay numbers, so this changes only a theoretical edge case.
- **Scalar-array semantics tightened, not loosened**: `structuresAgree` treats a scalar array as a set — a pure subset is enrichment, but a DIVERGENT pair (`[a,b]` vs `[a,c]`) stays a conflict, preserving the aggregator's exclusion-scope contract.

**Knock-on**:
- **Re-keying is a format migration**: re-aggregating a card produces new-format earn_rate groups next to any old-format ones. Per §12 this is done deliberately ONLY for the canary cards (subtask 8). The aggregator integration test re-runs on hsbc-red and therefore left 8 stale old-format earn_rate groups beside 9 new-format ones — a bounded, one-time test residue (idempotent upserts don't grow it). It affects only hsbc-red's /review queue; calculator, diagnose, and the canary are untouched. Non-canary group cleanup is deferred to review-queue triage / Stage 1B legacy backfill — we do NOT auto-delete non-canary data (mirrors §6's "reconcile via explicit action, don't auto-merge").
...
## D29 (P18 Stage 1A) — Canary write gates; annual_fee conflicts are materialization-eligible; gate deactivates stale rules

**Decision**: Three canary-surfaced refinements. (1) `scripts/materialize-canary.ts` is the ONLY Stage 1A materialization entry point and is dry-run by default; writing requires `--enable-write` + an on-allowlist `--card-slug` (Amex Explorer / Blue Cash only) + `--max-write-count N`, enforced by pure functions in `src/lib/extraction/canary.ts` (`parseCanaryArgs`, `validateCanaryConfig`, `enforceMaxWriteCount`) that a dry-run pass feeds a planned-write count before any write. (2) `materializeApprovedGroups` treats `annual_fee` groups as eligible even in `conflict` status — the §3E authority policy resolves the conflict itself (official source wins provisionally) — whereas reward-rule groups still require agreed/single_source. (3) When the inferred-category gate blocks a group, it also deactivates any pre-existing ACTIVE `xchk__` rule on the same (card, rule_type, category) — setting `is_active_for_calculator=false` + `publish_authority='reviewer_rejected'`.

**Why**:
- **§12 blast-radius**: "Do NOT allow an accidental Stage 1A command to update all cards." An explicit allowlist + a write budget computed from a mandatory dry-run pass makes a full-DB or oversized write impossible without three deliberate flags. The gates are pure + unit-tested so the guarantee doesn't depend on the DB.
- **Annual-fee conflict is the normal case, not an error**: Explorer's annual_fee group is `conflict` (official HK$2,200 vs outlier HK$9,500 vs first-year HK$0). The generic "conflict groups need a reviewer pick first" rule would leave the stale HK$1,800 published forever. The whole point of D25's authority policy is to resolve exactly this — so annual_fee must reach the writer despite the conflict verdict. Reward-rule conflicts keep the conservative gate (a wrong rate poisons the calculator; a provisional annual fee with a review task does not).
- **Blocking a new rule isn't enough (§3C "no active calculator rule should remain")**: Blue Cash already had an ACTIVE legacy `insurance = 1.2%` xchk rule materialized before the gate existed. The gate stops a *new* one but the old one keeps mis-earning. Deterministic reconciliation by (card, rule_type, category) — xchk only, never yaml — removes it, honoring §3C. A misclassification-caught-earlier gate + a stale-rule sweep together deliver the canary's "NO active insurance 1.2% rule".
- **cardholderType: primary marker wins over supplementary**: the official Explorer fee snippet reads "Basic Card Annual Membership fee HK$2,200 … Supplementary Card …". Checking supplementary first mis-tagged the official primary fee as supplementary, dropping it from the settable set and defeating the official-authority path. A primary marker (Basic Card / 主卡 / 基本卡) now wins when both appear — the extracted amount is the primary fee.

**Knock-on**: the write is fully reversible + canary-scoped; `scripts/audit-diff.ts` snapshots before/after for the §11 audit. The 9,500 outlier is currently `superseded` (aggregator), so Explorer lands `official_clean` → `provisional_pending_review` rather than the `_conflict_` variant; the outlier row is preserved (not rejected) for investigation. Non-canary cards are never touched (no reaggregate/materialize outside the allowlist).

## D30 (P18 Stage 1B core) — Persistent rule identity: stable scope key + deterministic 1:1 legacy backfill

**Decision**: Stage 1B core lands two additive tables in `src/db/schema/extraction.ts` (migration `0015_rule_identities.sql`) plus a pure, gated backfill. (1) `rule_identities` — immutable `id`, `card_id` (immutable anchor), nullable `origin_rule_id` → `reward_rules` (ON DELETE SET NULL), a deterministic `stable_scope_key`, `status` (CHECK: `legacy_unreconciled` default / `active` / `reconciled` / `retired`), and `audit_metadata` jsonb. (2) `reviewer_overrides` — bound to `rule_identity_id` + `field_path` + `version_scope` (NOT NULL default `''`), with a per-override `comparison_policy` (CHECK: `exact`/`numeric_abs`/`numeric_pct`/`set_equal`), `override_value`, `baseline_value`, `reason`, `reviewer_email`, and `status` (CHECK: `active`/`superseded`/`withdrawn`); a partial-unique index enforces ≤1 active override per (identity, field, scope). (3) `computeStableScopeKey` (in `src/lib/extraction/rule-identity.ts`) derives the key from STABLE dimensions only — `card_id`, `rule_type`, reward mode (`reward_formula_type` + `reward_currency_id`), `campaign_id` presence, and the three eligibility flags — versioned `v1|…`. (4) `planLegacyBackfill` maps each existing rule to exactly one identity, ordered by rule id, idempotent (skips rules whose id already has an identity), never merging same-key rules. `scripts/backfill-rule-identities.ts` runs it dry-run by default; writing needs `--enable-write` + `--max-write-count N`. Applied to the local DB: 213 rules → 213 `legacy_unreconciled` identities, 0 orphans, re-run is a no-op.

**Why**:
- **§6A/§12 — identity must survive row churn**: a `reward_rules` row is disposable (recrawl / re-materialization / row recreation / category correction / effective-date enrichment / source-priority changes). Overrides and future merge/split audit therefore bind to a stable `rule_identity_id`, never to the regenerated row UUID. The scope key deliberately EXCLUDES mutable/correctable fields (category, dates, source, confidence, notes) so a correction can't move identity.
- **D11 dependency direction**: both tables live in `extraction.ts` and reference catalog one-way. We deliberately did NOT add a `reward_rules.rule_identity_id` FK — that would make `catalog.ts` depend on the extraction namespace. The binding is owned via `origin_rule_id` and mirrored in `audit_metadata` so it outlives an ON DELETE SET NULL.
- **§6C — safe migration, never auto-merge**: the backfill is strict 1:1 and deterministic (sorted by rule id, no nondeterministic `LIMIT 1`). Two rules with the same scope key still get two identities; the collision is reported for a later explicit reviewer/matcher reconciliation, never collapsed. Over the live 213 rules this surfaced 41 collision groups — overwhelmingly `exclusion/no_reward` and `category_bonus/simple_percent` rules on one card that differ only by category (excluded from identity by §6A). This is expected: the eventual reconciliation matcher must use category as a NON-identity discriminator; the scope key alone cannot separate them, and must not try to.
- **§6E — field-specific override policy**: each override carries its own `comparison_policy` — there is deliberately no universal 5% threshold. `baseline_value` records what the reviewer saw so a later stale-override detector can flag drift.

**Knock-on**: additive + reversible (`DELETE FROM rule_identities` / drop `0015`); the calculator, YAML source-of-truth, canary path, and exclusion machinery are untouched (`pnpm diagnose` still green post-backfill). Migration `0015` was hand-authored + journal-appended (no drizzle snapshot), matching how `0012`–`0014` were done — the drizzle meta snapshot has been stale since `0011`, so `pnpm db:generate` is unusable here without first reconciling that drift (out of scope). **DEFERRED within Stage 1B** (NOT built this session, per the owner-scoped "core" package — see `CURRENT.md`): the `rule_identity_events` reversible merge/split table (§6D — the identity model preserves origin/audit so an append-only migration can add it with zero data loss), stale-override *detection* logic, the reconciliation matcher, and the durable review workflow. Migration `0015` + this backfill have NOT been applied to the live Supabase DB — that is a separate, owner-triggered deploy step.
